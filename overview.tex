...
\begin{algorithm}[h!]
 ...
 \For{$n=0,1,\dots,N$}{
  ...
  $\bm{X}^{0}=\bm{X}^{(n)}$, $\bm{U}^{0}=\bm{U}_p^{(n)}+ {\bm \Omega}_p^{(n)} \times \bm{R}$, $\bm{\Omega}_p^{0}=\bm{\Omega}_p^{(n)}$ \\
  ...
  \For{$q=1,2,3$}{ 
   ...
   \For{$s=0,1,\dots,N_s$}{
    ...
    $\bm{F}_{l}^{q-1/2,s} = \bm{F}_{l}^{q-1/2,s-1} + \frac{\bm{U}_{p}(\bm{X}_{l}^{q-1})-\bm{U}_{l}^{**,s-1}}{\Delta t} $ \\
    }
  ...
  $\bm{X}^{(n+1)}=\bm{X}^{3}$, $\bm{U}_p^{(n+1)}=\bm{U}_p^{3}$, $\bm{\Omega}_p^{(n+1)}=\bm{\Omega}_p^{3}$ \\
...
Regardless of the approach, the spreading of the Lagrangian force onto the Euler mesh is realized with regularized Dirac delta functions, which may take different forms and satisfy $\sum_{ijk} \delta_p( \bm{x}_{ijk}-\bm{X}_{l}) \Delta x \Delta y \Delta z=1$ for all $\bm{X}_{l}$.
To illustrate how it can be implemented, a pseudo code correspoinding to the direct forcing IB method for spherical particles using a third-order Runge-Kutta temporal integration is provided in Algorithm \ref{al:ibm}. There, the notation is similar to the previous formulations, cf.\ Eqs. \eqref{eq:projection-method} and \eqref{eq:lagrangian}, except that ${\bf RU}$ does not include $\bm f$; and some terms are simplified or omitted for ease of reading. See \cite{ Wim-Paul_JCP_2012} for the full details.
