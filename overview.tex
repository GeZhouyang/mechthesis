...
\begin{equation} \label{eq:surf-tension}
 ...
 \end{aligned}
\end{equation}
where $\bm{X}_l$, $l=1,2,\dots,N_L$ denote the positions of the Lagrangian points, $\bm u$ the local flow velocity, $\bm f$ the forcing term, $\gamma$ the surface tension coefficient, $\kappa$ the interface curvature, $\bm n$ the interface normal vector, and $\delta$ the Dirac delta function. Eq.\ \eqref{eq:surf-tension} correspoinds to the normal stress jump condition due to surface tension, cf.\ Eq.\ \eqref{eq:stress-bc}. No-slip boundary conditions on solid particles can be represented in similar ways as will be described momentarily.

Despite the conceptual simplicity, there are several technical details that require careful treatment in an implementation.
First, due to deformation of the particle or non-uniformity of the flow field, the Lagrangian particles may need to be redistributed, added, or deleted from time to time to ensure numerical stabilty and accuracy.
This is particularly cumbersome when droplets coalesce or breakup.
...
Third, the internal boundary conditions (no-slip or surface tension) are to be computed on the fixed mesh, thus a ``spreading'' procedure is nessesary to couple the particle and fluid dynamics. This is usually done with a smeared delta function.
...
\begin{algorithm}[h!]
 ...
 \For{$n=0,1,\dots,N$}{
  ...
  ${\bf RU}_{ijk}^0 = {\bf RU}_{ijk}^{(n)}$, ${\bf RU}_{ijk}^{-1} = {\bf RU}_{ijk}^{0}$  \\
  $\bm{X}_l^{0}=\bm{X}_l^{(n)}$, $\bm{U}_p^{0}=\bm{U}_p^{(n)}$, $\bm{\Omega}_p^{0}=\bm{\Omega}_p^{(n)}$ \\
...
To illustrate the overall algorithm, a pseudo code correspoinding to a direct forcing IB method for spherical particles using a third-order Runge-Kutta temporal integration is provided in Algorithm \ref{al:ibm}. We note that the notation is similar to the previous formulations, cf.\ Eqs. \eqref{eq:projection-method} and \eqref{eq:lagrangian}, except that ${\bf RU}$ does not include $\bm f$; and some terms are simplified or omitted for ease of reading. See \cite{ Wim-Paul_JCP_2012} for the full details.


\subsection{Front-capturing methods}

Instead of meshing and tracking the moving boundary on a separate grid, an alternative approach is to capture the particle motion on the same grid but with an additional function. Such function may correspond to the local volume fraction of a dispersed phase, or measure a geometric quantity at the local position relative to an interface. Conceptually, this is the \emph{Eulerian} specification of a given flow field, and many numerical methods fall under such framework. In the following, we will describe three such methods for ``front-capturing'' (see Figure \ref{fig:front-capture}). A brief historical background will be given first, followed by the definition of the front-capturing function and its governing equation. We will also highlight the major advantages and disadvantages of each method without going into much details. Finally, when coupled with a flow solver, the internal boundary conditions can be treated similarly as in front-tracking methods, cf.\ \eqref{eq:surf-tension}. We skip these details and refer the reader to \cite{Brackbill_JCP_1992, Fedkiw_JCP_1999, Lalanne_JCP_2015, Gibou_Fedkiv_Osher, ICLS} for further details.
...
\medskip
In summary...
\lb{add a generic pseudo code for all front-capturing methods.}
