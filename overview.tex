...
Theoretically, the resistence of a solid surface to fluid flows can be characterized by the \emph{slip length}, $\lambda_s$, defined as
\begin{equation}
 \begin{aligned}
...
In most situations, $\lambda_s$ is a constant material property whose value is very close to zero, or on the order-of-magnitude of the molecular interaction range under moderate shear \citep{Thompson_Troian_1997}.
...
Close inspection of the surface structure suggests that it is the patterning combined with their hydrophobic property that yields the apparent slip.
...
Under this simple description, many studies have been conducted to obtaining theoretical expressions of the effective slip for two-dimensional longitudinal or transverse grooves \citep{Lauga_Stone, Sbragalia_Prosperetti, Davis_Lauga, Ng_Wang, Schonecker, Nizkaya, Crowdy_tran, Crowdy_long}.
...
In general, different geometry- and viscosity-ratio-dependent expressions for $\lambda_e$ are found, depending on the configuration.

While these solutions are helpful in guiding the optimal surface desgin, they neglect an important ingredient -- wetting of the fluid.
...
Specifically, SHS is known to be sensitive to external pressure under which the entrapped bubble can collapse \citep{Bocquet}; whereas LIS is prone to shear-driven failures, \ie the process of the lubricant being drained from the surface grooves \citep{Wexler}.
Therefore, the dynamics is equally, if not more, important comparing to the static slippage property.
Unfortunately, precise control of wetting in experiments is still very challenging due to sensitivity to surface conditions; and the theoretical agreement remains inadequate \citep{deGennes_wetting}.
In light of these difficulties, numerical studies with controlled parameters in non-ideal situations are expected bridge the gap between analytical understandings and experimental findings.


\section{Droplets in turbulence}

Predicting the size of droplets or bubbles is of great importance ...
In oceans ... Breaking ocean waves entrain air bubbles that enhance air–sea gas flux, produce aerosols, generate ambient noise and scavenge biological surfactants \citep{deane_stokes_2002a}.
In clouds ... understanding how the size distribution variesspatially and temporally in a cloud is one of the major challenges in the atmosphericsciences.
the droplet size distribu-tion is the defining characteristic of a cloud, determining how the cloud interactswith  electromagnetic  radiation,  how  fast  precipitation  will  form,  and  so  forth. \citep{Shaw_2003}.
in emulsions is important in many industrial, biological, and environmental systems, as it determines the stability, rheology, and area available in the emulsion for physical or chemical processes that occur at the interface \citep{boxall_etal_2012}.

theoretical understanding \cite{hinze_1955a}.

also just to do some turbulence simulations.
...
In particular, we first show how the turbulent flow is modified by the droplets and how the droplets evolve in the turbulent flow, and then explain how these modifications occur by studying the turbulent kinetic energy balance in the two-phase flow. 
