...
However, the last few decades have also observed a phenomenal paradigm shift in many research areas owing to the exponential growth of computer power and continuous development of new algorithms.
...
The common ground at the bottom of these inquiries is that the processes all entail interactions of droplets in an ambient fluid or are governed by the dynamics near a fluid interface.
Their specific backgrounds are given below.
...
On the one hand, ideal PhC has the structure of a diamond cubic, which can have up to 40\% bandgap opening \citep{PRL1990}.\footnote{This percentage corresponds to the portion of frequencies that propogation of light is forbidden.}
Therefore, if one wishes to fabricate PhC from colloidal droplets, the holy grail to aim is positioning droplets in a diamond cubic with alternating dielectric constants.
...
The advantages of this \emph{flow-assisted} assembly strategy are the potential large production rate and robust control.
The questions remained to be answered include (i) why the droplets self-organize into those clusters and, if we understand that, (ii) how to harness this self-assembly mechnaism.
These questions are explored and answered in \emph{Paper 1-4}.



\iffalse
\section{Designing liquid-infused surfaces}
