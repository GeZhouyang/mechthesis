...
This chapter presents a selection of numerical methods that can be employed to solve the coupled fluid and particle dynamics.
First, we discuss standard Navier-Stokes solvers along with different interface-capturing methods. Simulations adopting these approaches intend to resolve all spatial and temporal scales within the system, and sometimes qualify as \emph{direct numerical simulations (DNS)}.
Then, we discuss a special class of methods that solves the particle dynamics without directly resolving the flow, thus much faster, but valid only in Stokes flows. Depending on the level of simplifications, these may be considered as \emph{coarse-grained} dynamics.
In view of the large amount of available numerical methods and the vast development of computational fluid dynamics in the last several decades, the list is by no means complete.
For more comprehensive surveys of various numerical techniques for multiphase flow problems, see \eg \cite{prosperetti_tryggvason_2007, Rosti2019}.
...
Discretization.
In space: finite difference, finite volume, finite element, spectral methods, \etc
In time: RK, AB2, CN, \etc
...
In a particle-based method, the central equation to solve is
\begin{equation} 
...
If the fluid inertia is also negligible, the Navier-Stokes equations reduces to the Stokes equations and $\bm{F}^H$ may be obtained without actually solving the fluid motions.
...
The other is derived from the former, with additional features to model particle contact; hence it is better suited for dense suspensions. We call the latter \emph{hybrid lubrication/granular dynamics} method.
...
The computational time depends on the algorithm and boundary conditions. In the best case, it is $\mathcal{O}(N_E\log N_E)$; while it can be $\mathcal{O}(N_E^3)$ if using direct solution methods \citep{graham_2018}.
...
\begin{equation} \label{eq:sd-mob-eq}
 ...
 \end{aligned}
\end{equation}
where $\mathscr{M}^\infty$ is the so-called ``grand mobility'' matrix, containing the structure of all hydrodynamic interactions per particle configuration.
...
There is one more crucial piece of the Stokesian dynamics method.
In the above construction of $\mathscr{M}^\infty$, lubrication interaction is not fully accounted for due to the omission of higher-order multipole moments (hence the superscript $^\infty$). To make a lubrication correction, $\mathscr{R}$ is calculated as
...
If the particle volume fraction is very high, say $\phi > 40\%$, the far-field, many-body hydrodynamic interaction will be less important than the pairwise, short-range lubrication force due to particle crowding. This justifies a further simplification of the mobility matrix by dropping all additional low-moment terms apart from the lubrication \citep{Ball_Melrose_1997}, \ie
\begin{equation} 
 \begin{aligned}
...
The detailed algebra is rather cumbersome. Here, we simply note that the components are essentially functions of the particles' velocities, size ratio, and surface gap ($h$). The diverging behaviour is strongest for the lubrication force in the normal direction ($F^{L,n} \sim 1/h$), while in the tangential directions, as well as for the lubrication torque, the divergence is in weaker form $\sim 1/\log(h)$. See \emph{Paper 7} for the detailed expressions.

For concentrated suspensions, lubrication interaction alone is not sufficient to reproduce certain experimental observations, such as the discontinuous shear thickening \citep{Morris_annurev2020}.
When particles are very closely packed, contact interactions such as collision and friction are inevitable due to surface roughness\footnote{Analytically, a diverging lubrication would prevent particle contact, though that neither agrees with everyday experience nor holds beyond the continuum limit.}
This motivates the merging of the reduced Stokesian dynamics with the granular dynamics approaches and has led to improved understanding of many of phenomena in dense suspensions \citep{Trulsson_Andreotti_Claudin, Mari_Seto_2014JoR, Cheal_Ness_2018}.
In the latter model, the granular dynamics is often implemented as a stick/slide friction model employing springs and dashpots \citep{Luding2008}.
These forces and torques are added in ${\bm F}^A$.
%In addition, for a computational standpoint,

Computationally, the solution procedure of the hybrid lubrication/granular dynamics is similar to Stokesian dynamics, except that there is no need to invert the mobility matrix and the resistance matrix is sparse. This leads to computational speedup.

Alternatively, the particle dynamics can also be obtained by explicitly solving Eq.\ \eqref{eq:force-balance}. That is, compute all forces and torques to get the acceleration, then integrate in time once to get the velocity and twice to get the position. 
In the colloidal context, such a approach is sometimes termed as dissipative particle dynamics or Brownian dynamcis \citep{Hoogerbrugge_1992, Groot_Warren_1997}.
Provided that the particle Reynolds number remains much less than unity, it is equivalent to the quasi-static matrix inversion approach.
