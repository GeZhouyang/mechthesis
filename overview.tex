...
Eq.\ \eqref{eq:multipole} is the \emph{multipole expansion} of the disturbance velocity, ${\bm u}-{\bm u}^\infty$. One immediate physical implication is that a forced particle (\eg a rising bubble) will generate a disturbance that decays as $1/r$, while a force-free particle (\eg a self-propelled bacterium) has a $1/r^2$ decay thus leaving less trace at the same distance away, where $r=\abs{{\bm x}}$. See Figure \ref{fig:lets} for an illustration.\footnote{The rotlet flow is simply a rigid-body rotation centered at the origin.}

\begin{figure}%[t]
  \centering
  \includegraphics[width=0.49\columnwidth]{stokeslet.pdf}
  \includegraphics[width=0.49\columnwidth]{stresslet.pdf}
  \caption{Flow fields due a Stokeslet (left) and a stresslet (right) centered at the origin (red dot). The black arrows display the velocities with their directions indicated by the gray arrows. Note the symmetries and the difference in velocity magnitudes of the two flows.}
  \label{fig:lets}
\end{figure}

Finally, for a sphere of radius $a$, translating with velocity $\bm U$, rotating with angular velocity $\bm \Omega$, its total drag, torque, and stresslet are given by the \emph{Fax\'{e}n's laws},
