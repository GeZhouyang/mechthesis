...
Motivated by these, the present work embarks on a number of problems that originates from material fabrication, surface engineering and fluid transport.
...
and large in magnitude (\ie $|N_2| \gtrapprox |N_1|$) due to particle collisions, which happen more frequently in the $xy$ plane \citep{guazzelli_pouliquen_2018}.
...
since the discretisation is required only on the particle surface instead of in the flow domain, cf.\ Section \ref{sec:fluid-methods}.
...
Provided that the particle Reynolds number remains much less than unity,
