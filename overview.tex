...
First proposed in the 1970s, the volume-of-fluid (VOF) method was one of the oldest interface-capturing methods for modelling of free-surface flow \citep{Debar1974, hirt_nichols_1975, hirt_nichols_1981a}, and it remains as one of the most widely used methods for multiphase flow simulations within the CFD community \citep{Scardovelli_Zaleski_1999, gerris, pilliod_puckett_2004, xiao_honma_kono_2005a, mthinc2012, paris2019}.
...
$\phi$ is then advected according to the following level set equation,
\begin{equation} \label{eq:ls-eq}
  \frac{\partial \phi}{\partial t} + F \abs{\nabla \phi} = 0,
\end{equation}
where $F$ is a speed function.
From a PDE perspective, the above equation prescribes that the entire level set evolution is obtained by integrating $\phi({\bm x},t=0)$ forward in time.
Hence, it is an initial value problem and numerical techniques developed for these problems are applicable.

Geometrically, the normal and the curvature of the interface can be readily derived from the level set, \viz
\begin{subequations}
 \begin{equation}
    {\bm n} = \frac{\nabla \phi}{\abs{ \nabla \phi }},
 \end{equation}
 ...
 \end{equation}
\end{subequations}
where the negative sign in Eq.\ \eqref{eq:curv} is due to convention.

When solving Eq.\ \eqref{eq:ls-eq} for droplet motions, the most natural choice for $F$ is the local flow velocity projected in the normal direction, \ie $F=\bm{u \cdot n}$. However, any non-uniform flow will distort the level set field, making Eq.\ \eqref{eq:ls-def} invalid. To remedy this problem, thus making the numerical solution stable, it is customary to perform a ``reinitialization'' of $\phi$ every few time steps \citep{Sussman_JCP_1994}.
...
comment on mass loss.

%coupled level set/VOF \citep{Sussman_JCP_2000}.
%hybrid particle level set method \citep{Enright_JCP_2002}.
%conservative level set \citep{Olsson_JCP_2005}.
