...
\begin{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.38\textwidth}
      ...
      \vskip.3cm
      Droplets are micron to millimeter sized liquid balls formed under surface tension.%
...
\begin{frame}[t]
  ...
  The coupled flow solver also requires accurate computation of {\bf surface tension} and a hydrodynamic model to impose the {\bf depletion force}.
...
\begin{frame}[t,noframenumbering]
  ...
  \only<3-5>{
    ...
    \begin{textblock}{140}(12,80)
      Figure: All initial positions (\xb{blue dots}) stabilize at a polar angle $\theta_\infty=$ 79 deg (\xr{red dot}).
    \end{textblock}    
  } % end only
  ...
  \only<6>{
    \begin{textblock}{95}(30,45)
        \begin{bluecolorbox}[Effects of depletion force $+$ shear $+$ confinement]
          ...
          \item It leads to alignment of a droplet pair in the flow direction. \medskip
  ...
  \begin{textblock}{80}(5,24)
    ...
    \only<3->{
      ...
      \item w/o depletion force $\rightarrow $ {\bf scattering} drops
        \only<4->{
        \item w/ depletion force $\rightarrow$ forming a \only<4-5>{chain \only<5>{or a triangle}}\only<6->{{\bf chain} or a triangle}
        } %end only
      \end{itemize}
    } % end only
    
    \vskip0.1cm
    \only<7->{In the region close to the inlet: non-uniform flow
      \only<8->{
        \begin{itemize}
        \item w/ depletion force $\rightarrow $ {\bf compact} clusters
        \end{itemize}
      }% end only
    } % end only
      ...
      \only<7->{
        \begin{bluecolorbox}[Photonic material fabrication]
          ...
          \item It's still possible to make PhC using the microfluidic-based approach,
            e.g.\ a trade-off approach has been proposed to assemble face-centered cubic composed of smaller clusters.%
...
\begin{frame}[t]
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.65\textwidth}
      ...
      \vskip0.2cm
      \only<4->{Broadly speaking, collections of cells, proteins, polymers, surfactants, colloidal grains, {\it etc.} are all examples of soft matter.}
      \vskip0.2cm
      ...
      \only<2->{
        ...
        \item Finite volume solvers %\vskip0.2cm
          \only<3->{
            \begin{itemize}
            \item accurate
            \only<4->{\item expensive since the \emph{whole domain} must be meshed}
            \end{itemize} \vskip0.1cm
          } %end only
          \only<5->{\item Boundary integral methods %\vskip0.2cm
            \only<6->{
              \begin{itemize}
              \item very accurate analytically
              \only<7->{\item potentially efficient as only \emph{surfaces} are to be meshed}
              \end{itemize} \vskip0.1cm
            } % end only
          } % end only
          \only<8->{
          \item Stokesian dynamics (SD) %\vskip0.2cm
            \only<12->{
              \begin{itemize}
              \item reduced accuracy
              \only<13->{\item improved efficiency as \emph{no meshing} is required}
              \end{itemize} \vskip0.1cm
            } % end only
          } % end only
        \only<16->{\item Hybrid lubrication/granular dynamics (HLGD)}
        \end{itemize}
      } % end only

      \only<17->{
        \begin{textblock}{90}(75,75)
        ...
        \begin{bluecolorbox}[Mathematical formulation]
          ...
          \only<5-7>{
            ...
            \begin{equation} \notag
              ...
              \end{aligned}
            \end{equation}
            Here, the velocity is expressed as integrals of known Green's function, summed over all particles.
          ...
          \only<8-9>{
            SD rewrites the previous integral as a multipole expansion,
            \begin{equation} \notag
              \begin{aligned}
            ...
            \only<9>{
              These terms are then be combined with the Fax\'{e}n's formulae for spheres
          ...
          \only<14->{
            In SD, the lubrication is explicitly accounted for in $\mathscr{R}$,
            \begin{equation} \notag
              \begin{aligned}
            ...
            \only<15->{ 
              However, for dense suspensions, $\mathscr{R}_{2B} \gg (\mathscr{M}^\infty)^{-1}$,
              since the particles are very close to each other and the lubrication diverges at contact.\vskip0.1cm
              This leads to a further simplification
              \begin{equation}  %\notag
                \begin{aligned}
              ...
              \vskip-.1cm
              \only<16->{
                Eq. (9), combined with models for contact forces (e.g.\ collision and friction), is the mathematical basis of HLGD.
              } %end only
            } %end only
          } %end only
...
\begin{frame}[t]
  ...
  \only<3->{
  We examine the jamming transition by plotting the
  \begin{itemize}
  \item relative viscosity, $\eta_r=\sigma_{xy}/(\mu\dot{\gamma})$,
  \item non-dimensional particle pressure, $\eta_n=-\textrm{tr}(\bm{\sigma})/(3\mu\dot{\gamma})$,
  \end{itemize}
  ...
  \only<3->{
    \begin{textblock}{40}(5,50)
      \includegraphics[width=\textwidth]{imgs/suspension-shear.png}\\
      {\footnotesize Image from \texttt{https://github.com/rmari/LF\_DEM}.}
    \end{textblock}
  } % end only

  \only<4->{
    \begin{textblock}{110}(46,44)
      \centering
      \includegraphics[width=0.48\textwidth]{../paper7/figs/curve_visc_fricless1.pdf}
      \includegraphics[width=0.48\textwidth]{../paper7/figs/curve_pp_fricless.pdf}
    \end{textblock}
  } % end only

  \only<5->{      
    \begin{textblock}{20}(124,40)
      \xb{$\phi_J \approx 65\%$}
    \end{textblock}
  } % end only
...
\begin{frame}[t]
  ...
  \begin{textblock}{80}(5,12)
    ...
    \vskip0.1cm
    \only<2->{
      We implement a critical-load friction model, with a build-in non-linearity.%
...
\begin{frame}[t]
  ...
  \begin{textblock}{140}(5,12)
    Finally, we simulate 500 particles in oscillatory shear flow at 40\% volume fraction.
    \vskip0.1cm
  ...
  \only<4>{
    ...
    \begin{textblock}{80}(80,32)
      ...
      \hskip1.5cm  Strong shear ($\dot{\gamma} \approx 10^5$)
    \end{textblock}
  } % end only
...
\begin{frame}[t]
  ...
  \begin{textblock}{130}(15,20)
    \begin{bluecolorbox}[Beyond spherical particles]
      ...
      \only<3->{However, for dense suspensions, a completely different rheology or phase transition may be expected
        as the lubrication intensifies and particle contact increases.} %end only
      \only<4->{
        \vskip0.2cm
