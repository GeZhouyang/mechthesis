...
%there many different models for dynamic wetting, based on hydrodynamic theories, molecular-kinetic theories, molecular dynamics, \etc \cite{Blake,...,GNBC}. None of these models are complete; however, censensus is still lacking. It shouldn't affect the overall qualitative results.
...
%based on heuristic modelling of the local slip length \cite{Schonecker}. 
...
%--%%Misc
%--%
%--%\bibitem{dimension} We estimate the dimensional shear rate at the upper boundary of our computational domian from the experimental Hele-Shaw setup in \cite{Wexler} (2 mL/min through a cross-section of 180 $\mu$m $\times$ 7 mm). Assuming a parabolic velocity profile, the reference velocity at $\tilde{H}=20$ $\mu$m is $\tilde{U}=$ 0.016 m/s, leading to a shear rate of $\dot{\tilde{\gamma}}=800$ s$^{-1}$. The characteristic contact line velocity is calculated as $\tilde{U}_c=\tilde{\sigma}/\tilde{\mu}_2$, taking $\tilde{\sigma}=0.02$ kg/$s^2$ and $\tilde{\mu}_2=0.0076$ $\nicefrac{kg}{ms}$, as in \cite{Martin}.
%--%
%--%
%--%\end{thebibliography}}
%--%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
