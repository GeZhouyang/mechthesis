...
Among the early studies of suspension hydrodynamics, \cite{batchelor_green_1972} solved completely the dynamics of two rigid spheres of arbitrary size ratio subject to a background fluid motion whose velocity at infinity is a linear function of position, \ie
...
In the special case of two equal-sized spheres suspended in a simple shear flow, the relative velocity can be integrated numerically to yield a trajectory diagram as in Figure \ref{fig:BG-traj}.
...
It was noted by the authors that, under such ideal conditions (\eg smooth surface), the separatrix between open and closed trajectories encloses an infinite volume.
Furthermore, if the two particles are once aligned in the flow direction, they will form a closed loop.
