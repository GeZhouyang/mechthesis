...
\begin{enumerate}
 \item \emph{Linearity}. This immediately renders the superposition principle applicable. For example, if $({\bm u}_1,p_1)$ is a solution to ${\bm f}={\bm f}_1$ and $({\bm u}_2,p_2)$ is a solution to ${\bm f}={\bm f}_2$, then $(\alpha{\bm u}_1+\beta {\bm u}_2,\alpha p_1 + \beta p_2)$ is a solution to ${\bm f}=\alpha{\bm f}_1 + \beta {\bm f}_2$.
...
If $\bm f$ in Eq.\ \eqref{eq:Stokes} is a point force exerted at position ${\bm x}_0$, \ie ${\bm f}=\delta({\bm x}-{\bm x}_0)$, the solution of the Stokes equation in unbounded domain reads,
\begin{equation}
 \begin{aligned}
   {\bm u}({\bm x}) = {\bm G}({\bm r}) \cdot {\bm F},
 \end{aligned}
\end{equation}
where ${\bm r}={\bm x}-{\bm x}_0$ and 
The free-space Green's function: the Stoekslet,
