...
\begin{frame}[t]
  ...
  \only<2->{
    \begin{textblock}{70}(5,20)
      A pair of droplets in unbounded simple shear flow can have two types of trajectories%
      ...
      \end{itemize}
      \vskip0.1cm
      These two classes of trajectory is only determined by the relative position for equal spheres. 
    \end{textblock}
...
\begin{frame}[t,noframenumbering]
  ...
  \begin{textblock}{140}(5,15)
    ...
    \only<2->{
      Without the depletion attraction, droplets separate and swap their positions (i.e.\ symmetrical).%
      \footnote<.->[frame]{M. Zurita-Gotor \etal J. Fluid. Mech. 592, 447-469. (2007). I. Fouxon \etal ArXiv:1908.11208.}
    } %end only
    \only<3->{
      \vskip0.1cm
      With the depletion attraction and strong confinement, only one configuration is dynamically stable (i.e.\ the symmetry is lost).
    } %end only      
  \end{textblock}
  ...
  \only<3-4>{
    \begin{textblock}{135}(7,35)
      \begin{figure}
        ...
        \only<4>{\includegraphics[width=.417\columnwidth]{../paper2/figs/traj.pdf}}
...
\begin{frame}[t]
  ...
  \begin{textblock}{80}(5,24)
    \only<2->{In the region far away from the inlet... \vskip0.1cm
      \begin{itemize}
      \item w/o depletion force $\rightarrow $ scattering drops
        \only<3->{
        \item w/ depletion force $\rightarrow$ forming a chain \only<4->{or a triangle}
        } %end only
      \end{itemize}
    } % end only
    ...
    \only<6->{In the region close to the inlet...
      \begin{itemize}
      \item Non-uniform inflow \only<7->{$\rightarrow $ compact clusters}
      \end{itemize}
    } % end only
  ...
  \only<8>{
    \begin{textblock}{73}(5,52)
      \begin{bluecolorbox}[Effects of \emph{all} combined]
        ...
        \item Homogeneous shear flow with confinement promotes droplet chains. %\medskip
        \item Non-uniform inflow can lead to compact structures. %\medskip
    ...
    \begin{column}{0.95\textwidth}
      \begin{bluecolorbox}[Droplet interactions]
        ...
        \item The depletion force is short-range, radial and instantaneous.
          ...
          \only<4->{
          \item Overall, it is difficult to calibrate all 3D effects at play. \medskip}
        \end{itemize}
      \end{bluecolorbox}
        ...
        \begin{bluecolorbox}[Photonic material fabrication]
          ...
          \item The microfluidic-based approach is theoretically possible, but tricky in practice.%
            ...
            \item Current trend seems to be shifted from colloidal self-assembly to making new disordered structures.%
              \footnote<2->[frame]{Ricouvier \etal PNAS. 116 (19), 9202-9207 (2019).}    \medskip}
          \end{itemize}
        \end{bluecolorbox}
      } %end only
    \end{column}
