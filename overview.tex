...
For non-Brownian suspensions, several particle diffusivities may be defined, though the motion of a particle is typically not diffusive except at short and long times \citep{Brady_Bossis1988}. In the long-time limit, the self-diffusivity ${\bm D}_\infty^s$ is defined as 
\begin{equation}
 \begin{aligned}
   {\bm D}_\infty^s = \lim_{t \to \infty} \frac{1}{2} \frac{d}{dt} \bigg \langle \big({\bm x}_i - \langle {\bm x}_i \rangle \big)^2 \bigg \rangle,
 \end{aligned}
\end{equation}
where ${\bm x}_i$ is the position of particle $i$, and $\langle \cdot \rangle$ denotes the ensemble average.

\subsection{Stress tensor}

One of the central tasks of rheology is to determine the bulk stress tensor $\langle \bm{\Sigma} \rangle$ of a flowing material
\begin{equation} 
  \begin{aligned}
    \langle \bm{\Sigma} \rangle = -\bm{\Pi} +2\mu \bm{E}^\infty + \frac{N}{V} 
    \bigg(\langle \bm{S}^H \rangle + \langle \bm{S}^B \rangle +\langle \bm{S}^A \rangle  \bigg), 
  \end{aligned}
\end{equation} 
where $V$ is the enclosing volume, $N$ is the number of particles, and $\bm{S}$'s denote the stresslets due to various particle interactions, including the hydrodynamic ($H$), Brownian ($B$), and any other applied force ($A$), as written.
Apart from the viscous stress tensor, $-\bm{\Pi} +2\mu \bm{E}^\infty$, due to the underlying flow, the rest of the terms are the particle contribution.
In general, these stress components are functions of the suspension microstructure.
In the case of the hydrodynamic stresslet, it can be computed as
\begin{equation} 
  \begin{aligned}
    \langle \bm{S}^H \rangle = - \langle \bm{R}_{SU}\cdot(\bm{U}-\bm{U}^\infty) - \bm{R}_{SE}:\bm{E}^\infty \rangle , 
  \end{aligned}
\end{equation}
where $\bm{R}_{SU}$ and $\bm{R}_{SE}$ are configuration-dependent resistance matrix that has the structure of $\bm{M}^{-1}$. For a further discussion of these computations, see Section \ref{subsec:sd}.
%In a dynamic simulation, they may be equivalently expressed with the particle velocities, as velocities and positions are uniquely determined from each other in the overdamped limit. See Section \ref{subsec:sd} for the explicit formula of these stresslets.

Now, we describe some useful definitions for rheology.
\begin{enumerate}
\item Effective viscosity

Very dilute suspension (up to $\phi \approx 0.05$) \citep{Einstein_1906, Einstein1911}
\begin{equation}
 \begin{aligned}
   \eta_r = 1 + 2.5\phi.
 \end{aligned}
...
Semi-dilute suspension (up to $\phi \approx 0.15$) \citep{batchelor_green_1972b}
\begin{equation}
 \begin{aligned}
   \eta_r = 1 + 2.5\phi + 6.95\phi^2.
 \end{aligned}
\end{equation}

some empirical fits, Eiler?
see \cite{Brady_Bossis1988, guazzelli_pouliquen_2018} for reviews.

\item Particle pressure

boyle's model

\item Normal stress differences
\item Shear thickening, shear thinning

something interesting to the industry \cite{Morton_Morris_2014, Morris_annurev2020}
