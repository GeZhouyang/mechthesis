...
This chapter summarizes the mathematical formulation and basic theoretical ideas of microhydrodynamics and particle interactions.
...
As preliminaries, most of the contents are given without any proof. The interested reader is encouraged to read \cite{Batchelor, hb, ps, graham_2018} for greater details.
...
Depending on the bounding material, conditions on the velocity or stress may be prescribed as the boundary condition. In the case of a solid wall, it is usually the \emph{no-slip} velocity condition; while in the case of a liquid interface, the following stress condition shall be satisfied,
...
When particles are present in the fluid, the coupled dynamics is often significantly more complex than a single phase Stokes flow. Therefore, theoretical analyses are usually limited to simplified or idealized conditions. Below, we try to gain some intuitions by considering the simplest case, \ie a single particle in unbounded Stokes flows.

Imagine the force density ${\bm f}$ in Eq.\ \eqref{eq:Stokes} is singularly supported at the origin, \ie ${\bm f}({\bm x})={\bm F}\delta({\bm x})$, $\delta$ being the Dirac delta. This can correspond to a small sedimenting particle viewed from a distance. Then the solution of the Stokes equation due to this forcing can be written as
...
\begin{equation}
 \begin{aligned}
   {\bm G}({\bm x}) = \frac{1}{8\pi\mu } \bigg( \frac{{\bm I}}{\abs{\bm x}}+\frac{ {\bm r}{\bm r} }{ \abs{{\bm x}}^3 } \bigg),
 \end{aligned}
\end{equation}
is the free-space Green's function, also known as the \emph{Stokeslet}.

Now, if the particle has a finite radius $a$ and the background fluid has a linear velocity ${\bm u}^\infty$ in the absence of the particle, we can perform a Taylor expansion of $\bm G$ and integrate Eq.\ \eqref{eq:stokes-green} over the particle surface to obtain a solution of the following form (in index notation)
\footnote{In this simplified example, we require the point ${\bm x}$ to be far from the particle so that the terms involving the Green's function can be taken outside the integral, see \cite{graham_2018}.}
\begin{equation} \label{eq:multipole}
 ...
 \end{aligned}
\end{equation}
where ${\bm F}^{d}$ denotes the total drag force exerted on the particle by the fluid, and ${\bm D}$ is an integral expression representing the force dipole.
%\begin{equation} 
% \begin{aligned}
...
\begin{subequations}
 \begin{equation}
   T_{jik}^{STR}({\bm x}) = -\frac{1}{8\pi\mu} \frac{3x_i x_j x_k}{\abs{{\bm x}}^5},
 \end{equation}
 \begin{equation}
   T_{jik}^{ROT}({\bm x}) = -\frac{1}{8\pi\mu} \frac{\delta_{ik} x_j - \delta_{ij} x_k}{\abs{{\bm x}}^3}.
 \end{equation}
\end{subequations}

Eq.\ \eqref{eq:multipole} is the \emph{multipole expansion} of the disturbance velocity, ${\bm u}-{\bm u}^\infty$. One immediate physical implication is that a forced particle (\eg a rising bubble) will generate a disturbance that decays as $1/r$, while a force-free particle (\eg a self-propelled bacterium) has a $1/r^2$ decay thus leaving less trace at the same distance away.

Finally, for a sphere of radius $a$, translating with velocity $\bm U$, rotating with angular velocity $\bm \Omega$, its total drag, torque, and stresslet are given by the Fax\'{e}n's laws,
\begin{subequations} \label{eq:faxen}
 ...
 \end{equation}
\end{subequations}
where ${\bm E}^\infty$ is the rate-of-strain tensor \citep{Batchelor}.
...
confinement (as another particle)
