...
For example, recall the example as considered in the beginning of Section \ref{sec:stokes-flows}, and suppose it is an oil droplet traveling in water (the surface tension coefficient would be around 0.05 N/m, depending on surfactants and the temperature) under a relatively large local shear rate of $\dot{\gamma}=10^3$ s$^{-1}$, then the capillary number Ca $=\mu(a\dot{\gamma})/\gamma \sim 10^{-4}$, which is small enough for the droplet to remain spherical in the flow.
...
\begin{figure}%[t]
  ...
  \caption{
  (left) Trajectory map of two equal spheres in simple shear flows bounded by a distant wall. The distance between the reference particle (located at the origin) and the wall is $z_0=5a$, where $a$ is the particle radius. $r_s$ and $r_c$ denote the saddle point and the neutral equilibrium point, respectively.
...
When walls are present, particles not only interact with each other but also indirectly interact with the wall, or equivalently, interact with their reflection images such that the boundary conditions along the wall are satisfied \citep{blake_1971, LironMochon}.
Consequently, pair interactions in bounded flows may differ at least quantitatively from the unbouned conditions.

We first consider the simplest modification -- the inclusion of one wall.
Using an boundary integral representation (\emph{Paper 3}), we recently solved the relative velocity of two rigid particles in simple shear flows above a distant wall (\emph{Paper 4}).
The results are illustrated in Figure \ref{fig:itzhak-boris}.
Here, the blue curves denote the closed trajectories, and the red curves denote the open ones.
First, we see there are two disconnected closed regions; one being a degenerate of the BG trajectory, the other indicating rotation around a center (which we call ``dancing'' trajectories).
The latter suggest the existence a stationary position.
Note that the trajectories are axisymmetric. Thus, particles can travel together without relative motions in a whole range of configurations.
The intersection between the blue and red curves represents the unstable saddle point.
Furthermore, the red curves indicates previously identified ``swapping trajectories'' \citep{zurita-gotor_2007}.
Overall, the phase portrait of two particles in wall-bounded shear flows is topologically different from BG.

\begin{figure}%[t]
  \centering
  \includegraphics[width=0.55\columnwidth]{stresslet_reflection.png}
  \caption{Swapping mechanism.}
  \label{fig:stresslet-refl}
\end{figure}

Some mentioning of the swapping mechanism.
With two walls, \cite{zurita-gotor_2007}

also talk about my particle array.
crossover from trapping to passing.
