...
\begin{frame}[t]
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.4\textwidth}
      ...
      \only<2->{
        \begin{itemize}
        \item Well-resolved NS solvers (meshing the entire space) \vskip0.2cm
        \only<4->{\item Boundary integral methods (meshing the particle surface) \vskip0.2cm}
        \only<5->{\item Stokesian dynamics (SD), solving (meshless)
          \begin{equation} 
            \begin{aligned} \notag
        ...
        \only<10->{\item Hybrid lubrication/granular dynamics (HLGD).}
        \end{itemize}
      } % end only

      \vskip0.4cm
      \only<11->{\xb{\bf We implemented a version of HLGD to study DS.}}
    \end{column}
      ...
      \only<2->{
        \begin{bluecolorbox}[Mathematical formulation]
          ...
          \only<4-5>{
            ...
            \only<5>{
              SD rewrites the r.h.s of Eq.\ (9) as a multipole expansion,
              \begin{equation} \notag
                \begin{aligned}
                  - \frac{1}{8\pi \mu} \sum_{\alpha=1}^N  \int_{S_\alpha} ... \approx \bm{u}_F + \bm{u}_S + \bm{u}_T + \bm{u}_Q + \bm{u}_O , 
                \end{aligned}
              \end{equation}
              where $\bm{u}$'s denote the flow due to leading-order poles.
            } % end only
          } % end only
          ...
          \only<6-7>{
            ...
            \only<7>{
              ...
              \begin{equation} \notag
                ...
                \end{aligned}
              \end{equation}
              which, once obtained, can be used to calculate $\bm U$.
            } % end only
          } % end only
